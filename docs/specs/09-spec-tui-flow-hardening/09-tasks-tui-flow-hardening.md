...
- Proof file: `docs/specs/09-spec-tui-flow-hardening/09-proofs/task-1-ui-server-concurrency.md` captures the command outputs above

#### 1.0 Tasks

- [ ] 1.1 Run plan 016's drift check (`git diff --stat 2ff060e..HEAD -- <plan in-scope files>`); create branch `advisor/016-ui-server-concurrency`
- [ ] 1.2 Add `syncStatusCache` to `ui_actions.go` with constructor-injected fetch (`newSyncStatusCache(fetch func() tea.Msg)`), 30s TTL, `cmd()` and `invalidate()` (plan 016 Steps 1+5 note)
- [ ] 1.3 Wire the cache into `dashboardModel` (field, init, replace `dashboardSyncStatusCmd()` call sites, invalidate in `startAction`) and into `uiServer` (field, init, `status` case, invalidate at the start of scan/refresh/plan/apply/hydrate) (Step 2)
- [ ] 1.4 Add action-closure test seams to `uiServerOptions` (`scanCmd`, `refreshCmd`, `planCmd`, `applyCmd`, `hydrateCmd`, `statusCmd`), defaulted in `runUIServer`, used by `handle` (Step 3)
- [ ] 1.5 Dispatch every request on its own goroutine; add `beginAction`/`endAction` single-flight (immediate `busy: <label> in progress` rejection, `defer endAction` after successful begin); update the stale sequential-handling comment, keep the unbounded-ReadString rationale and EOF behavior (Step 4)
- [ ] 1.6 Write `TestUIServerReadsNotBlockedBySlowAction`, `TestUIServerRejectsConcurrentActions`, and `TestSyncStatusCache` TTL/invalidate tests using the existing io.Pipe harness pattern (Step 5)
- [ ] 1.7 Run gates (`go test -race` selection, `make verify`), write proof file `09-proofs/task-1-ui-server-concurrency.md`, flip plan 016's row to DONE in `plans/README.md`, commit
...
- Proof file: `docs/specs/09-spec-tui-flow-hardening/09-proofs/task-2-protocol-contract.md` captures the command outputs above
...
- [ ] 2.6 Run gates (`make verify`, `make tui-verify`), spot-check that deleting a fixture field fails `bun test` (restore it), write proof file `09-proofs/task-2-protocol-contract.md`, flip plan 017's row in `plans/README.md`, commit
...
- Proof file: `docs/specs/09-spec-tui-flow-hardening/09-proofs/task-3-client-watch-robustness.md` captures the command outputs above
...
- [ ] 3.8 Run gates (`make verify`, `make tui-verify`, race-enabled Go selection), write proof file `09-proofs/task-3-client-watch-robustness.md`, flip plan 018's row in `plans/README.md`, commit
...
- Proof file: `docs/specs/09-spec-tui-flow-hardening/09-proofs/task-4-tui-install.md` captures the command outputs above (placeholder tokens only)
...
- [ ] 4.6 Run gates (`make verify`, `go run ./cmd/devspace tui install --help`), write proof file `09-proofs/task-4-tui-install.md`, flip plan 019's row in `plans/README.md`, commit; note in the proof file that the one-time manual smoke against a real release (plan 019 test plan) remains for a human with repo access
