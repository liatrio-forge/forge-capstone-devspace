...
run "$devspace" sync remote create local "$manifest_remote"
push_a="$("$devspace" sync push)"
printf '%s\n' "$push_a"
...
log "Machine B: pull manifest, plan, apply, and update projects"
export DEVSPACE_HOME="$home_b"
run "$devspace" init --workspace "$workspace_b"
run "$devspace" sync remote set "$manifest_remote"
pull_b="$("$devspace" sync pull)"
printf '%s\n' "$pull_b"
assert_contains "$pull_b" "Pulled workspace manifest."
plan_b="$("$devspace" plan)"
printf '%s\n' "$plan_b"
assert_contains "$plan_b" "PLACEHOLDER work/client-a-api"
[[ -f "$workspace_b/.devspace/last-plan.json" ]] || fail "expected saved plan at $workspace_b/.devspace/last-plan.json"
...
[[ -z "$(find "$workspace_b/work/client-a-api" -mindepth 1 -print -quit)" ]] || fail "placeholder directory should be empty before project update"
...
env_write="$("$devspace" env write client-a-api)"
printf '%s\n' "$env_write"
...
project_status="$("$devspace" status client-a-api)"
printf '%s\n' "$project_status"
